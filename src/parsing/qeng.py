    ...
    for values in parse_response(response, "+QENG"):
        ...
        if values[0] == "LTE" or values[2] == "LTE":
            ...
            try:
                ...
                corrected_sinr_value = raw_sinr * 2 - 20
                ...
                lte_cell = ServingCellQENG(
                    ...
                    cell_id=parse_optional_int(values[4], hex=True),
                    pci=parse_int(values[5]),
                    arfcn=parse_int(values[6]),
                    band=parse_int(values[7]),
                    ul_bandwidth=LTE_BW_INDEX_TO_MHZ.get(parse_int(values[8])),
                    dl_bandwidth=LTE_BW_INDEX_TO_MHZ.get(parse_int(values[9]), 0),
                    tac=parse_optional_int(values[10], hex=True),
                    rsrp=parse_int(values[11]),
                    rsrq=parse_int(values[12]),
                    rssi=parse_int(values[13]),
                    sinr=corrected_sinr_value,
                    cqi=parse_int(values[15]),
                    tx_power=parse_int(values[16]) / 10,
                )
            ...
            try:
                nsa_cell = ServingCellQENG(
                    ...
                    dl_bandwidth=NR_BW_INDEX_TO_MHZ.get(parse_int(values[9]), 0),
                    scs=NR_SCS_INDEX_TO_KHZ.get(parse_int(values[10])),
                )
            ...
            try:
                sa_cell = ServingCellQENG(
                    ...
                    cell_id=parse_optional_int(values[7], hex=True),
                    pci=parse_int(values[8]),
                    tac=parse_optional_int(values[9], hex=True),
                    arfcn=parse_int(values[10]),
                    band=parse_int(values[11]),
                    dl_bandwidth=NR_BW_INDEX_TO_MHZ.get(parse_int(values[12]), 0),
                    rsrp=parse_int(values[13]),
                    rsrq=parse_int(values[14]),
                    sinr=parse_int(values[15]),
                    scs=NR_SCS_INDEX_TO_KHZ.get(parse_int(values[16])),
                )
